...
def _today_local_date() -> date:
    # Ajusta si tu server no está en TZ local; mantenemos portable y estable
...
def _line_for_today(db: Session, proyeccion_id: int, today: date) -> Optional[ProyeccionLinea]:
    """
    Hotfix: elegir la línea de proyección más cercana a 'today' (pasado o futuro).
    Antes: tomábamos la última <= hoy o la primera en general (que podía estar lejísima).
    """
    prev_line = (
...
def _densidad_base_org_m2(db: Session, plan: Optional[SiembraPlan], estanque_id: int) -> Optional[Decimal]:
    """
    Hotfix 1: solo usar override si es > 0; si no, caer a plan si es > 0; de lo contrario, None.
    Esto evita contaminar KPIs con ceros/valores inválidos.
    """
    if not plan:
        return None
    se = (
...
def _pond_operational_row(
    *,
    ...
    if dens_base is None:
        # Si prefieres ver filas aunque falte densidad, aquí podríamos devolver una fila con None.
        return None
    ...
    if sob_pct is not None and pp_g is not None:
        org_vivos_est = (dens_rem * Decimal(str(estanque.superficie_m2)) * (sob_pct / Decimal("100"))).quantize(Decimal("0.0001"))
    ...
    return {
        ...
        "densidad_remanente_org_m2": float(dens_rem),
...
def _aggregate_kpis(pond_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for r in pond_rows:
        if r["densidad_remanente_org_m2"] is not None and r["superficie_m2"] is not None:
            sup = Decimal(str(r["superficie_m2"]))
            dens = Decimal(str(r["densidad_remanente_org_m2"]))
            superficies += sup
            dens_x_sup += (dens * sup)
            ponds_with_dens += 1

        if r["org_vivos_est"] is not None:
            org = Decimal(str(r["org_vivos_est"]))
            org_vivos_tot += org
            if r["sob_vigente_pct"] is not None:
                sob_x_org += (Decimal(str(r["sob_vigente_pct"])) * org)
            if r["pp_vigente_g"] is not None:
                pp_x_org += (Decimal(str(r["pp_vigente_g"])) * org)
            ponds_with_org += 1

        if r["biomasa_est_kg"] is not None:
            biomasa_total += Decimal(str(r["biomasa_est_kg"]))

    dens_prom = (dens_x_sup / superficies) if superficies > 0 else None
    ...
    return {
        ...
        "densidad_remanente_prom_org_m2": float(dens_prom.quantize(Decimal("0.0001"))) if dens_prom is not None else None,
...
def _alerts(
    ...
    window_days_for_pp_dev: int = 7,  # Hotfix: gate por cercanía de la proyección
) -> List[Dict[str, Any]]:
    alerts = []

    # 1) recencia de biometría > 14 días
    for r in pond_rows:
        last = (
    ...
    # 2) desvío PP vigente vs proyección de la semana (solo si la línea está cerca de hoy)
    proj = _current_projection(db, ciclo_id)
    if proj:
        line = _line_for_today(db, proj.proyeccion_id, today)
...
def operational_state(db: Session, user: Usuario, ciclo_id: int) -> Dict[str, Any]:
    ...
    for pond in ponds:
        ...
        pp_g, pp_src, pp_ts = _current_pp_g(db, ciclo_id, pond.estanque_id, today)
    ...
    alerts = _alerts(db, ciclo_id, pond_rows, today)
